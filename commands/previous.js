...
module.exports = {
    name: "previous",
    description: "Switches to the previous Song",
    async execute(message, args) {
        if (musicQueue[message.member.guild.id]){
            ...
            if (musicQueue[message.member.guild.id]['playingIndex'] > 0){
                ...
                message.channel.send(outputEmbed.createOutputEmbed("Queue", `You are already at the beginning of the queue`)).then(sentMessage => {
                    setTimeout(() => {
                        sentMessage.delete();
                    }, config.messageDeleteDelay);
                });
